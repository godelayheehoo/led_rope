...
// Animation speed: a pulse launched on beat 1 reaches the end in one measure (96 MIDI clocks)
...
const float PULSE_DECAY_RATE_STOPPED = 0.03f; // Natural decay rate of active pulses when paused
...
void renderPulses() {
  ...
  for (int pIdx = 0; pIdx < MAX_PULSES; pIdx++) {
    if (!pulses[pIdx].active) continue;
    ...
    if (pulses[pIdx].isDownbeat) {
      pulseColor = CRGB(255, 255, 255); // Downbeat accent
    } else {
      pulseColor = CRGB(0, 150, 255);   // Standard beat visualizer
    }
    ...
    // Head has a linear fade in from +1 LED, trail has an exponential decay behind
...
void handleMidiByte(uint8_t byte) {
  if (byte == 0xF8) { // MIDI Timing Clock
    ...
    isPlaying = true;
    // Resume without clearing or triggering overlays
  }
}
...
void loop() {
  ...
  if (currentMillis - lastFrameTime >= FRAME_INTERVAL) {
    ...
    // If playback is stopped, decay active pulses so they fade naturally
    if (!isPlaying) {
      for (int i = 0; i < MAX_PULSES; i++) {
        if (pulses[i].active) {
